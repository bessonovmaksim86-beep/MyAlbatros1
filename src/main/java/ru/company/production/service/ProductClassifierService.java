...
import ru.company.production.dto.SystemComponentForm;
...
public class ProductClassifierService {
    ...
    @Transactional(readOnly = true)
    public List<ProductClassifier> deviceOptions(Long excludedClassifierId) {
        return classifiers.findDeviceOptions(excludedClassifierId);
    }
    ...
    public ClassifierForm getForm(Long id) {
        ...
        for (ClassifierInclusion inclusion : classifier.getInclusions()) {
            ...
            row.setMaxQuantity(inclusion.getMaxQuantity());
            row.setUnit(inclusion.getUnit());
            row.setInclusionMode(inclusion.getInclusionMode());
            rows.add(row);
        }
        form.setInclusions(rows);
        List<SystemComponentForm> componentRows = new ArrayList<>();
        for (SystemComponent component : classifier.getSystemComponents()) {
            SystemComponentForm row = new SystemComponentForm();
            if (component.getSensorCatalog() != null) {
                row.setSensorCatalogId(component.getSensorCatalog().getId());
            }
            if (component.getComponentClassifier() != null) {
                row.setComponentClassifierId(component.getComponentClassifier().getId());
            }
            row.setQuantity(component.getQuantity());
            row.setMaxQuantity(component.getMaxQuantity());
            row.setUnit(component.getUnit());
            row.setInclusionMode(component.getInclusionMode());
            componentRows.add(row);
        }
        form.setSystemComponents(componentRows);
        return form;
    }
    ...
    public void create(ClassifierForm form) {
        ...
        replaceSystemRows(form, classifier);
        save(classifier);
    }
    ...
    public void update(Long id, ClassifierForm form) {
        ...
        classifier.clearSystemComponents();
        classifiers.flush();
        replaceRows(form, classifier);
        replaceSystemRows(form, classifier);
        save(classifier);
    ...
    private void replaceRows(ClassifierForm form, ProductClassifier owner) {
        ...
        for (ClassifierInclusionForm row : rows) {
            ...
            inclusion.setMaxQuantity(row.getMaxQuantity());
    ...
    private void replaceSystemRows(ClassifierForm form, ProductClassifier system) {
        List<SystemComponentForm> rows = selectedSystemRows(form);
        if (rows.isEmpty()) return;
        if (system.getProductType() != ProductType.SYSTEM) {
            throw new IllegalArgumentException("Состав системы задаётся только для системы");
        }
        Set<Long> sensorIds = new HashSet<>();
        Set<Long> deviceIds = new HashSet<>();
        for (SystemComponentForm row : rows) {
            validateSystemRow(row);
            if (row.getSensorCatalogId() != null) {
                if (!sensorIds.add(row.getSensorCatalogId())) {
                    throw new IllegalArgumentException("Род датчика повторяется в составе системы");
                }
            } else {
                if (Objects.equals(system.getId(), row.getComponentClassifierId())) {
                    throw new IllegalArgumentException("Система не может входить сама в себя");
                }
                if (!deviceIds.add(row.getComponentClassifierId())) {
                    throw new IllegalArgumentException("Прибор повторяется в составе системы");
                }
            }
        }
        Map<Long, SensorCatalog> sensorRefs = new HashMap<>();
        if (!sensorIds.isEmpty()) {
            sensors.findAllById(sensorIds).forEach(s -> sensorRefs.put(s.getId(), s));
            if (sensorRefs.size() != sensorIds.size()) {
                throw new EntityNotFoundException("Один из родов датчиков не найден");
            }
        }
        Map<Long, ProductClassifier> deviceRefs = new HashMap<>();
        if (!deviceIds.isEmpty()) {
            classifiers.findAllById(deviceIds).forEach(d -> deviceRefs.put(d.getId(), d));
            if (deviceRefs.size() != deviceIds.size()) {
                throw new EntityNotFoundException("Один из приборов не найден");
            }
            for (ProductClassifier device : deviceRefs.values()) {
                if (device.getProductType() != ProductType.DEVICE) {
                    throw new IllegalArgumentException("В состав системы входят только роды датчиков и приборы");
                }
            }
        }
        int position = 0;
        for (SystemComponentForm row : rows) {
            ...
            if (row.getSensorCatalogId() != null) {
                component.setSensorCatalog(sensorRefs.get(row.getSensorCatalogId()));
            } else {
                component.setComponentClassifier(deviceRefs.get(row.getComponentClassifierId()));
            }
            component.setQuantity(row.getQuantity());
            component.setMaxQuantity(row.getMaxQuantity());
            component.setUnit(row.getUnit());
            component.setInclusionMode(row.getInclusionMode());
            component.setPosition(position++);
            system.addSystemComponent(component);
        }
    }

    private List<SystemComponentForm> selectedSystemRows(ClassifierForm form) {
        if (form.getSystemComponents() == null) return List.of();
        return form.getSystemComponents().stream().filter(Objects::nonNull)
                .filter(row -> row.getSensorCatalogId() != null
                        || row.getComponentClassifierId() != null)
                .toList();
    }

    private void validateSystemRows(ClassifierForm form, Long currentClassifierId) {
        List<SystemComponentForm> rows = selectedSystemRows(form);
        if (rows.isEmpty()) return;
        if (form.getProductType() != ProductType.SYSTEM) {
            throw new IllegalArgumentException("Состав системы задаётся только для системы");
        }
        Set<Long> sensorIds = new HashSet<>();
        Set<Long> deviceIds = new HashSet<>();
        for (SystemComponentForm row : rows) {
            ...
            if (row.getSensorCatalogId() != null) {
                ...
                if (Objects.equals(currentClassifierId, row.getComponentClassifierId())) {
                    throw new IllegalArgumentException("Система не может входить сама в себя");
                }
                if (!deviceIds.add(row.getComponentClassifierId())) {
                    throw new IllegalArgumentException("Прибор повторяется в составе системы");
    ...
    private void validateSystemRow(SystemComponentForm row) {
        if (row.getSensorCatalogId() != null && row.getComponentClassifierId() != null) {
            throw new IllegalArgumentException("Компонент системы: выберите род датчика или прибор");
        }
        if (row.getSensorCatalogId() == null && row.getComponentClassifierId() == null) {
            throw new IllegalArgumentException("Выберите компонент состава системы");
        }
        ...
        if (row.getMaxQuantity() != null
                && row.getMaxQuantity().compareTo(row.getQuantity()) < 0) {
            throw new IllegalArgumentException(
                    "Максимальное количество не может быть меньше фактического");
        }
        if (row.getInclusionMode() == null) throw new IllegalArgumentException("Выберите режим включения компонента");
    ...
    private void validateAllowedTargetType(ProductType ownerType, ProductType targetType) {
        boolean allowed = switch (ownerType) {
            ...
            /* Состав системы хранится отдельно в system_components. */
            case SYSTEM, CELL -> false;
        };
    ...
    private void validate(ClassifierForm form, Long currentClassifierId) {
        ...
        validateSystemRows(form, currentClassifierId);
    }
